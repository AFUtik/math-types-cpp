...
struct transform {
    ...
    >
    static constexpr inline void translate(matrix<T, MN, MM> &model, const vector<V, N> &vector) {
    ...
    template <
        ...
        typename V, std::size_t N
    >
    static constexpr inline void setPosition(matrix<T, MN, MM> &model, const vector<V, N> &vector) {
        static_assert(MM >= 1 || MN >= 1, "Size must be greater than one.");
        constexpr std::size_t POS_INDEX = (MN*MM)-MN;
        for(uint32_t i = POS_INDEX, j = 0; j < N; i++, j++) model.data[i] = vector.data[j];
    };
    ...
    >
    static constexpr inline void setRotation(matrix<T, MN, MM> &model, const matrix<V, N, M> &matrix) {
